...
public class Logic {
    ...
    private final Image game_background = new ImageIcon("Candy_Crush/res/bg-candy.jpg").getImage();
    private final Image win_gif = new ImageIcon("Candy_Crush/res/oie_OeMAOSwhprac.gif").getImage();
    private final Image out_of_moves_image = new ImageIcon(/* out of move imange */).getImage();
    private final Image next_icon_big = new ImageIcon("Candy_Crush/res/close_button.png").getImage();
    ...
    private final Image Dispenser = new ImageIcon("Candy_Crush/res/Dispenser.png").getImage();
    ...
    public Logic(int candies_width, int candies_height, int moves_count, int AmountOfCandies) {
        ...
        try {
            Candy_font = Font.createFont(Font.TRUETYPE_FONT, new File("Candy_Crush/res/CANDY.TTF")).deriveFont(30f);
            GraphicsEnvironment ge = GraphicsEnvironment.getLocalGraphicsEnvironment();
            ge.registerFont(Font.createFont(Font.TRUETYPE_FONT, new File("Candy_Crush/res/CANDY.TTF")));
    ...
    public void draw(Graphics g) {
        ...
        for (Point obstacle : obstacles) {
            ...
            g.drawImage(obstacleImage, obstacle.x * 50 + 325, obstacle.y * 50 + 150, 50, 50, null);
        }
    }
        ...
        if (match_checker()){
            for (int i = 0; i < candies_width; i++) {
                for (int j = 0; j < candies_height; j++) {
                    ...
                    if (j < candies_height - 2 && candies[i][j].getCandyType() == candies[i][j + 1].getCandyType()
                            && candies[i][j].getCandyType() == candies[i][j + 2].getCandyType() && obstacle == ) {
                        return true;
                    } else if (i < candies_width - 2 && candies[i][j].getCandyType() == candies[i + 1][j].getCandyType()
                            && candies[i][j].getCandyType() == candies[i + 2][j].getCandyType()) {
                        return true;
